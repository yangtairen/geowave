...
public class BitmaskUtilsTest
{
	...
	public void testByteSize() {
		...
		Assert.assertTrue(
				1 == zeroth.toByteArray().length);
		Assert.assertTrue(
				2 == eighth.toByteArray().length);
	}

	@Test
	public void testGetOrdinal() {
		List<Integer> positions = BitmaskUtils.getFieldPositions(
				zeroth.toByteArray());
		Assert.assertTrue(
				0 == positions.get(
						0));
		Assert.assertTrue(
				1 == positions.size());
		positions = BitmaskUtils.getFieldPositions(
				first.toByteArray());
		Assert.assertTrue(
				1 == positions.get(
						0));
	...
	public void testCompositeBitmask() {
		...
		final byte[] bitmask = BitmaskUtils.generateCompositeBitmask(
				new TreeSet<Integer>(
						Arrays.asList(
								0,
								1,
