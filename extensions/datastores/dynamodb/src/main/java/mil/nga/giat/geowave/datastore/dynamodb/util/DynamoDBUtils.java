...
public class DynamoDBUtils
{
	...
	public static <T> T decodeRow(
		...
		final ByteBuffer idBuf = row.get(
				DynamoDBDataModel.GW_ID_KEY).getB();
		...
		final GeowaveRowId rowId = new GeowaveRowId(
				row.get(
						DynamoDBDataModel.GW_IDX_KEY).getB().array(),
				dataId,
				adapterId,
				1);
	...
	public static <T> Pair<T, DataStoreEntryInfo> decodeRow(
		...
		final Set<FieldReader<? extends CommonIndexValue>> visitedIndexModelReaders = new HashSet<>();
