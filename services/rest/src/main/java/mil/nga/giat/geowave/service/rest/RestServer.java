		...
		ServerResource
{
	...
	public RestServer(){
		...
		for (Class<?> operation : new Reflections("mil.nga.giat.geowave")
				.getTypesAnnotatedWith(GeowaveOperation.class)) {
		    availableRoutes.add(new Route(operation));
		}
	}
	...
	public void run(int port){
	    ...
	    for (Route route : availableRoutes) {
			if (route.isServerResource()){		
				router.attach(route.getPath(),route.getOperationAsResource());
			} else {
				router.attach(route.getPath(),NonResourceCommand.class);
			}
		}
	...
	private static class Route {
		 ...
		 * @param operation
		 */
		public Route(Class<?> operation){
			this.path = pathFor(operation).substring(1);
			this.operation = operation;
			
			// check if operation extends ServerResource, which is required for Restlet to 
		...
		/**
		 * @return true if the route represents an operation which extends ServerResource
		 */
		...
		/**
		 * Return the operation as its ServerResource subclass if operation extends
		 * ServerResource, otherwise return null.
		 * 
		 * @return the operation as its ServerResource subclass, or null if operation
		 * does not extend ServerResource
		 */
		...
		/**
		 * Get the path for a command based on the operation hierarchy
		 * Return the path as a string in the format "/first/next/next"
		 * 
		 * @param operation - the operation to find the path for
		 * @return the formatted path as a string
		 */
		public static String pathFor(Class<?> operation ) {
